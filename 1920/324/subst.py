...
def __build (env, target, source):
    ...
    dict = env.Dictionary()
    regexp = env.get ("SUBST_REGEXP", None)
    if regexp is None:
        for l in input.readlines():
            output.write (l % dict)
    else:
        if not hasattr (regexp, "sub"):
            regexp = re.compile (regexp)
        def sub (match):
            return dict[match.group (1)]
        for l in input.readlines():
            output.write (regexp.sub (sub, l))
...
def __emit (target, source, env):
    ...
    for src in source:
        ...
        if regexp is None:
            ...
            if not hasattr (regexp, "findall"):
                regexp = re.compile (regexp)
            for match in regexp.findall (src.get_contents()):
                for tgt in target:
                    env.Depends (tgt, env.Value (env[match]))
...
def register (env):
    ...
    class Wrapper:
        ...
        def __call__ (self, *args, **kwargs):
            self.method (self.env, *args, **kwargs)
    env.Wrap (Defined   = Wrapper (env, __Defined),
              Undefined = Wrapper (env, __Undefined))
